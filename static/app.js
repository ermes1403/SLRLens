const state = { dataset: null, result: null, excluded: new Set(), activeTopic: 1, topicAlgorithm: "lda" };
...
function renderResults() {
  ...
  $("#result-kpis").innerHTML = [
    ...
    [topicLabel, result.best_topic_number, `${result.document_count} documenti`],
    ["Coerenza", fmt(result.coherence_npmi, 3), "NPMI ↑"],
    ["Stabilità", `${fmt(result.stability * 100, 1)}%`, `${result.methodology.random_seeds.length} seed`],
...
$$(".result-tabs button").forEach(button => button.addEventListener("click", () => {
  ...
  $$(".tab-panel").forEach(element => element.classList.toggle("active", element.id === `tab-${button.dataset.tab}`));
  if (button.dataset.tab === "map") requestAnimationFrame(drawDocumentMap);
}));
...
function renderTopicSummary() {
  $("#topic-summary").innerHTML = state.result.topics.map((topic,index) => `
...
function renderTopicExplorer() {
  ...
  const topics = isLsi ? state.result.lsi.topics : state.result.topics;
...
function renderQuality() {
  ...
  $("#quality-diagnostics").innerHTML = [
    diagnosticRow("Stabilità multi-seed", state.result.stability, `${fmt(state.result.stability*100,1)}%`, .65),
    diagnosticRow("Topic diversity", state.result.topic_diversity, `${fmt(state.result.topic_diversity*100,1)}%`, .7),
    diagnosticRow("Esclusività", state.result.topic_exclusivity, `${fmt(state.result.topic_exclusivity*100,1)}%`, .55),
...
function renderBibliometrics() {
  ...
  $("#topic-impact").innerHTML = metrics.topic_impact.map((topic,index) => `<div class="impact-card" style="--topic-color:${colors[index%colors.length]}"><small>TOPIC ${topic.topic} · ${topic.papers} PAPER</small><strong>${fmt(topic.citations,0)}</strong><span>citazioni · media ${fmt(topic.mean_citations,1)}</span></div>`).join("");
...
function renderCandidateTable() {
  ...
  $("#candidate-table").innerHTML = `
    ...
    <table class="paper-table"><thead><tr><th>K</th><th>LDA NPMI ↑</th><th>LSI NPMI ↑</th><th>LDA stabilità</th><th>LSI stabilità</th><th>LDA perplexity ↓</th><th>LSI varianza</th><th>Repliche</th><th>Esito</th></tr></thead><tbody>${
